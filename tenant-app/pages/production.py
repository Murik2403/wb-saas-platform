...
def render(ctx: dict) -> None:
    ...
    render_section_header(
        "План производства",
        "Что и сколько нужно произвести, исходя из заказов и текущих остатков.",
    )
    st.caption(
    ...
    if enabled_settings.empty:
        ...
        render_section_header(
            "Потребность по материалам и цветам",
            "Сколько сырья каждого материала/цвета нужно, чтобы выполнить план производства.",
        )
        st.caption(
        ...
        render_section_header(
            "Календарный план",
            "Производственный план, разложенный по дням с учётом мощности и сроков поставки.",
        )
        st.caption(
        ...
        render_section_header(
            "План отгрузок",
            "Когда и сколько готовой продукции нужно отгрузить на склад WB.",
        )
        st.caption(
        ...
        render_section_header(
            ...
            "Конкретные действия на сегодня: что изготовить, что отгрузить, что скорректировать в рекламе.",
        )
        st.caption(
        ...
        render_section_header(
            "Исполнение плана",
            "Закрытие смен, отгрузки, печатные формы, журнал движений и себестоимость партий — "
            "фактическое выполнение производственного плана.",
        )
        st.caption(
            "Фиксируйте фактический выпуск и отгрузки. Операции проводятся отдельными безопасными "
        ...
        with execution_tabs[5]:
            render_section_header(
                "НЗП / заготовки",
                "Незавершённое производство: выпуск заготовок из сырья и их упаковка в готовые комплекты.",
                level=4,
            )
            st.caption(
            ...
            with wip_tabs[0]:
                render_section_header(
                    "Выдать сырьё и выпустить заготовки",
                    "Списывает сырьё со склада и создаёт партию заготовок незавершённого производства.",
                    level=5,
                )
                st.caption(
                ...
                render_section_header(
                    "Закрыть ранее открытую партию",
                    "Завершает партию заготовок, ранее выпущенную, но ещё не закрытую.",
                    level=5,
                )
            ...
            with wip_tabs[1]:
                render_section_header(
                    "Текущие остатки незавершённого производства",
                    "Сколько заготовок сейчас в работе, ещё не упаковано в готовые комплекты.",
                    level=5,
                )
            ...
            with wip_tabs[2]:
                render_section_header(
                    "Упаковать комплекты из заготовок",
                    "Превращает заготовки незавершённого производства в готовые к отгрузке комплекты.",
                    level=5,
                )
                st.warning(
            ...
            with wip_tabs[3]:
                render_section_header(
                    "Движения незавершённого производства",
                    "Журнал всех операций с заготовками: выпуск, упаковка, отмены.",
                    level=5,
                )
        ...
        render_section_header(
            "Очерёдность производства",
            "В каком порядке лучше производить товары с учётом дефицита и приоритета.",
        )
        plan_columns = [
