...
def render(ctx: dict) -> None:
    ...
    with stock_tabs[1]:
        ...
        if valuation.empty:
            ...
            st.caption("Готовый склад, товар в пути и остаток WB оцениваются по своим FIFO-слоям. Непокрытая слоями часть оценивается по базовой себестоимости. Новые продажи после перехода на послойный учёт списываются по конкретным FIFO-слоям; исторические операции остаются на базовой оценке.")
    ...
    with stock_tabs[4]:
        ...
        st.caption(
            "Этот режим разделяет обычную локальную сверку и расхождения на стороне WB. "
    ...
    with stock_tabs[5]:
        ...
        if incident_diag.empty:
            ...
            st.info(
                ...
                f"FBW-сверка включает в баланс {confirmed_fbw_total} ед. из подтверждённых API принятых FBW-поставок после базового снимка, "
            ...
            st.warning(
                "FBW-сверка уже вычитает подтверждённые API поставки FBW из необъяснённого притока, но не создаёт для них FIFO-слои автоматически. "
            ...
            st.caption(
                "FBW-сверка сохраняет результат проверки FBW API в локальной базе. Поставка со статусом 5 («принято») и factDate после базового снимка "
            ...
            render_section_header(
                "Ретроспективная реконструкция FIFO (preview)",
            ...
            if not retro_summary.get("ready"):
                ...
                st.warning(
                    f"Количество и даты FBW-поставок подтверждены API. В этом предпросмотре для стоимости используется лучший доступный исторический cost basis "
                ...
                if not retro_rows.empty:
                    ...
                    st.caption(
                        "Это только предпросмотр (dry-run). Кнопки применения реконструкции намеренно нет: сначала проверяем хронологию и итог по SKU, "
                        "а затем отдельно определяем надёжный источник себестоимости для мостового слоя и подтверждённых FBW-поставок."
                    )

            render_section_header(
                "Реконструкция себестоимости исторических слоёв",
                "Пересчитывает себестоимость уже существующих исторических FIFO-слоёв там, где она была неверной или отсутствовала.",
                level=4,
            )
            st.caption(
                "Этот шаг не применяет реконструкцию. Для каждой единицы мостового слоя и подтверждённой FBW-поставки он выбирает "
            ...
            if retro_summary.get("ready"):
                ...
                if actual_units < basis_units:
                    st.warning(
                        ...
                        "Поэтому на этом шаге кнопка применения реконструкции намеренно отсутствует."
                    )
                elif basis_units > 0:
                ...
                st.info("Cost basis станет доступен после готовности ретроспективного replay.")

            render_section_header(
                "Финальный аудит доказательств перед Apply",
                            ...
                            if not token:
                                ...
                                if saved:
                                    st.session_state["wb_supply_reconstruction_flash"] = (
                                        f"Аудит проверил и сохранил {saved} дополнительн. incomeID. Replay и финальный аудит пересчитаны."
                                    )
                                    st.rerun()
            ...
            st.caption(
                "Это первый этап, который реально меняет FIFO. Перед записью этот шаг автоматически создаёт консистентную SQLite-копию, "
            ...
            if not v67_suspense.empty:
                ...
                st.warning(
                    "Apply Reconstruction допускает cost_pending только для узкого suspense-исключения: единица без каталога/ставки и без продаж, которая остаётся в WB-контуре вне доступного остатка. "
            ...
            if v67_ready.get("ready"):
                ...
                if st.button(
                    ...
                    try:
                        ...
                        st.session_state["wb_v67_flash"] = (
                            f"Apply Reconstruction применён: run #{applied.get('run_id')}; FIFO {num(int(applied.get('fifo_units',0) or 0))} ед.; "
                ...
                with st.expander("Аварийный полный откат", expanded=False):
                    st.warning(
                        "Откат восстанавливает ВСЮ SQLite-базу из автоматической копии, созданной непосредственно перед Apply. "
                        "Все изменения данных после Apply тоже будут потеряны. Перед откатом дополнительно сохраняется safety-backup текущего состояния."
                    )
                ...
                if verified_rows:
                    ...
                    if confirmed_count:
                        st.success(
                            ...
                            "Эти поставки сохранены в базе и включены в количественную реконструкцию. "
                    ...
                    if goods_rows:
                        with st.expander("Состав проверенных поставок", expanded=False):
                            ...
                            st.caption(
                                ...
                                "FBW-сверка использует эти единицы как подтверждённые входящие в количественной реконструкции, но не создаёт стоимостный FIFO-слой."
                            )
        ...
        if cases.empty:
            ...
            st.warning(
                ...
                "Даже прямой вызов функции не позволит опустить FIFO ниже контура. Утрата проводится отдельным движением «wb_incident_loss», "
                ...
                if loss_candidates.empty:
                    ...
                    if blocked_units > 0 or layer_shortfall > 0:
                        st.warning(
                            ...
                            "Система не создаёт их автоматически без достоверного источника себестоимости."
                        )
