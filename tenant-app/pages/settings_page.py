...
def render(ctx: dict) -> None:
    ...
    with tabs[0]:
        render_section_header(
            "Токен WB API",
            "Персональный токен доступа к API Wildberries — без него дашборд не сможет получать данные.",
        )
        ...
        render_section_header(
            ...
            "Client-Id и Api-Key для агента цен Ozon. Реклама Ozon использует отдельные ключи Performance API — их сюда вставлять не нужно.",
        )
        st.caption(
    ...
    with tabs[1]:
        render_section_header(
            "Обновление данных",
            "Частота автоматической синхронизации с WB и ручной запуск, если данные выглядят устаревшими.",
        )
    ...
    with tabs[2]:
        render_section_header(
            "Себестоимость",
            "Себестоимость каждого товара — автозаполняется, но стоит сверить вручную, "
            "так как от неё зависит расчёт маржи и прибыли.",
        )
        ...
        st.divider()
        render_section_header(
            "Текущая себестоимость и прогноз новой партии",
            "Действующая себестоимость по товару и оценка, какой она будет при следующей закупке.",
            level=4,
        )
        st.info(
    ...
    with tabs[3]:
        render_section_header(
            ...
            "Типы заготовок, материалы/цвета и связанные с ними настройки расчёта потребности в сырье.",
        )
        st.caption(
    ...
    with tabs[4]:
        render_section_header(
            "Остатки сырья",
            "Текущий остаток каждого вида сырья на складе, с себестоимостью по FIFO.",
        )
        st.caption(
        ...
        st.divider()
        render_section_header(
            "Послойная стоимость сырья — FIFO",
            "Каждая партия закупленного сырья хранится отдельным слоем со своей ценой — расход "
            "списывается по методу FIFO (сначала пришло — сначала списано).",
            level=4,
        )
        st.caption(
        ...
        st.divider()
        render_section_header(
            "Послойная стоимость готовой продукции — FIFO",
            "То же самое, но для готовых товаров: себестоимость каждой партии хранится отдельно "
            "и списывается по FIFO при продаже.",
            level=4,
        )
        st.caption(
        ...
        render_section_header(
            ...
            "Себестоимость каждой продажи и возврата, рассчитанная по слоям FIFO — основа для точного расчёта прибыли.",
            level=4,
        )
        st.caption(
        ...
        st.divider()
        render_section_header(
            "Контроль расхождений FIFO",
            "Проверяет, не разошлись ли остатки в системе с данными Wildberries, и подсказывает, что делать при расхождении.",
            level=4,
        )
        st.caption(
            "Этот режим отделяет безопасную локальную сверку от диагностических расхождений WB. "
    ...
    with tabs[5]:
        render_section_header(
            "Готовая продукция и поставки на WB",
            "Остаток готовой продукции и товары, уже отправленные на склад WB, но ещё не принятые.",
        )
        st.caption(
    ...
    with tabs[6]:
        render_section_header(
            ...
            "Сколько единиц в день вы можете произвести и сроки поставки — используется при расчёте плана и сроков.",
        )
        st.caption(
    ...
    with tabs[7]:
        render_section_header(
            ...
            "Автоматические ежедневные бэкапы базы данных и возможность восстановить из них состояние.",
        )
        st.caption(
        ...
        render_section_header(
            "Восстановление",
            "Откатывает базу данных к выбранной резервной копии. Необратимо — используйте с осторожностью.",
            level=4,
        )
